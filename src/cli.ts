...
program
  .name('scrapeslayer')
  .description('ScrapeSlayer - Advanced social media scraping tool for Twitch and YouTube creator pages')
  .version('1.0.0');
...
program
  ...
  .description('Scrape Twitch creator pages')
...
program
  .command('youtube')
  .argument('[input]', 'YouTube username or URL to scrape')
  .option('-f, --format <format>', 'Output format (json|markdown)', 'json')
  .option('-o, --output <file>', 'Output file (default: stdout)')
  .option('-b, --batch <creators>', 'Comma-separated list of creators')
  .option('--file <path>', 'File containing list of creators (one per line)')
  .description('Scrape YouTube creator pages')
  .action(async (input: string | undefined, options) => {
    const format = options.format as OutputFormat;
    ...
    try {
      ...
      if (hasInput) {
        // Single creator mode
        console.error(`Scraping YouTube: ${input}`);
        const result = await scraper.scrapeYouTubeCreator(input!);
        ...
        // Batch mode - would need to implement YouTube batch processing
        console.error('Batch mode for YouTube not yet implemented');
        process.exit(1);
      }
